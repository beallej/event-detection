...
INSERT INTO feeds (feed_name, source, url, scrapers) values ('CNN_US', (SELECT id FROM sources WHERE source_name = 'CNN'), 'http://rss.cnn.com/rss/cnn_us.rss', '{"CNN"}'),
	('REUTERS_TOP_NEWS', (SELECT id FROM sources WHERE source_name = 'REUTERS'), 'http://feeds.reuters.com/reuters/topNews', '{"REUTERS"}'), ('ABC_TOP_STORIES', (SELECT id FROM sources WHERE source_name = 'ABC'), 'http://feeds.abcnews.com/abcnews/topsto    ries', '{"ABC"}');
