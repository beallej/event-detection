...
line="$java_path -jar $(pwd)/event-detection.jar $(pwd)/configuration.json && $python_path $(pwd)/Daemons/ArticleProcessorDaemon.py && $python_path $(pwd)/Daemons/ValidatorDaemon.py"
line="0 * * * * $line"
